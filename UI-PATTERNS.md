...
<tr key={`${r.original.code}-${index}`} className="border-t border-slate-200 align-top">
  ...
  {/* No disabled: visual override classes — disabled state is handled by the browser default */}
  <Td>
    <input
      type="text"
      value={r.description}
      ...
      className="w-full h-8 border border-slate-300 rounded px-2 text-sm bg-white focus:outline-none"
    />
...
| Input (edit/create) | `w-full h-8 border border-slate-300 rounded px-2 text-sm bg-white focus:outline-none` |
