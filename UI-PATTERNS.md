...
// RESPONSIVE RULES (apply to every table):
// 1. Always use overflow-x-auto on the scroll wrapper — never clip content on small screens
// 2. Always set min-w on Th so columns don't collapse
// 3. Action/button columns: always add whitespace-nowrap to prevent wrapping
// 4. Secondary numeric columns (PD, counts, codes): hide on smaller screens with
//    responsive Tailwind prefixes (hidden lg:table-cell) to free space for primary columns
// 5. Primary columns (name, description, actions): always visible on all screen sizes
...
<div className="bg-white border border-slate-200 rounded overflow-hidden">
  ...
  {/* overflow-x-auto is REQUIRED — never remove it */}
  {/* NEVER use inline-block min-w-max — it prevents expanding to fill space */}
  {/* Use w-full + min-w-[Npx] on BOTH the header div AND the table */}
  {/* This ensures: expand to fill space when available, scroll when too narrow */}
  <div className="overflow-x-auto w-full">
    <div className="px-3 py-2 bg-[#1F3864] text-white text-sm font-semibold
      border-b border-slate-200 min-w-[700px] w-full">
      [Section Title]
    </div>
    <table className="min-w-[700px] w-full text-sm">
      <thead className="bg-[#1F3864] text-white">
        <tr>
          ...
          {/* Secondary columns — hide on small screens to prevent overflow */}
          <Th className="min-w-[80px] hidden lg:table-cell">Count</Th>
          {/* Action column — always visible, always whitespace-nowrap */}
          <Th className="min-w-[80px] whitespace-nowrap">Actions</Th>
        </tr>
      </thead>
      <tbody>
...
| Table scroll wrapper | `relative overflow-x-auto` — **REQUIRED on every table, never remove** |
| Secondary columns (PD, counts, codes) | Add `hidden lg:table-cell` to both `<Th>` and `<Td>` to hide on small screens |
| Action button columns | Always add `whitespace-nowrap` to prevent text wrapping |
