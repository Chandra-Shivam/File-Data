...
Define these once at the top of the page file (above the default export).
  ...
  <div className="overflow-x-auto">
    <table className="w-full text-sm">
      ...
      <tbody className="divide-y divide-slate-100">
        ...
        {/* Loading state — skeleton rows inside tbody */}
        {isLoadingRows && <SkeletonRows />}
...
Define at the top of the page file alongside `Th` and `Td`.

```tsx
// PATTERN: SkeletonRows — shown inside tbody while data is loading
  ...
  return (
    <>
      {Array.from({ length: 5 }).map((_, i) => (
        <tr key={i}>
          <Td colSpan={5}>
            <div className="h-10 w-full bg-slate-100 border border-slate-200 rounded animate-pulse" />
          </Td>
        </tr>
      ))}
    </>
...
<tr key={r.original.code}>
  ...
  <Td className="font-mono text-slate-700">{r.original.code}</Td>
