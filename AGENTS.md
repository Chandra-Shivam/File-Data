...
- **Controller return types:** Existing controllers may return domain types directly (e.g. `CovenantLibraryItem`) for simple data shapes — this is acceptable and must not be refactored unless explicitly asked. For all **new** endpoints, return an explicit Application-layer contract from `Reviews/Contracts/`. Do not return raw domain types from new endpoints.
...
|---|---|---|
| Success (GET) | 200 OK | DTO / contract object |
| Success (POST/Create) | 201 Created | Created DTO |
| Success (PUT/Update) | 204 No Content | Empty body |
...
1. Page title (`<h1>`)
2. Filter bar — dropdowns to filter the table; changing a filter **triggers an API re-fetch via useEffect** watching the filter state (not client-side filtering)
...
- Delete: clicking **Delete** opens a **modal overlay with accessibility attributes** (role="dialog", aria-modal, focus trap, ESC key) — never inline row confirm, never `window.confirm()`.
...
- **Non-dismissible error banner** at top of page on any API failure — clears automatically after the next successful action (do not add a close/dismiss button).
...
| C# DTO / contract | PascalCase — no enforced suffix. Both `LoanCodeContract.cs` and `ReviewFormSaveModels.cs` patterns are acceptable. Match the naming of surrounding files in `Reviews/Contracts/`. | `LoanCodeContract.cs` or `LoanCodeModels.cs` |
...
- ❌ Never use `window.confirm()` for delete confirmations — always use the modal pattern from UI-PATTERNS.md Section 9
