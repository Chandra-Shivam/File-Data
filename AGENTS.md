...
- Register new repositories in `Program.cs` in `Casrr.Api`:
  add `// DEVELOPER: Register in Program.cs` comment on the interface declaration.
...
### Backend
- [ ] Register `I<Entity>Repository` → `Sql<Entity>Repository` in Program.cs (Casrr.Api)
